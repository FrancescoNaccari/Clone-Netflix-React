// src/components/modal/MovieModal.jsx
...
const MovieModal = ({ show, handleClose, movie, genresMap }) => {
  ...
  const loadMovieDetails = async (movieId) => {
    try {
      ...
      const filteredSimilarMovies = similar.results
        .filter((item) => item.poster_path)
        .map(mapSimilarMovieData);
  ...
  const getGenresString = (genreIds) => {
    if (!genresMap) return '';
    return genreIds.map((id) => genresMap[id] || 'Sconosciuto').join(' • ');
  };
          ...
          <Modal.Body>
            ...
            <div className="movie-details mt-4 px-4">
              ...
              <div className="movie-info-line">
                <span>Anno {selectedMovie.release_date ? new Date(selectedMovie.release_date).getFullYear() : 'ND'}</span><br />
                <span>{selectedMovie.runtime ? `${selectedMovie.runtime} min` : 'ND'}</span>
              </div>
              <p className="movie-description mt-1">
                      ...
                      >
                        <div className="poster-container">
                          ...
                          <div className="movie-details-overlay mt-1 d-flex justify-content-between pe-1">
                            <span>{similarMovie.release_date !== 'ND' ? new Date(similarMovie.release_date).getFullYear() : 'ND'}</span> 
